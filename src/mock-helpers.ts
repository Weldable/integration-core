...
export function deriveSeed(parentSeed: string, salt: string | number): string {
...
/** Plausible HTTPS URL. */
export function fakeUrl(seed: string, path?: string): string {
  const r = rand(seed)
...
export function fakeId(seed: string, len = 12): string {
 ...
 */
export function fakeSlackTs(seed: string): string {
  const r = rand(seed)
 ...
 */
export function fakeIsoTimestamp(seed: string, opts?: { offsetMs?: number }): string {
  const r = rand(seed)
 ...
 */
export function fakeArray<T>(seed: string, n: number, fn: (itemSeed: string) => T): T[] {
