 ...
 */
function synthesizeField(field: OutputField, rand: () => number): unknown {
  ...
  switch (field.type) {
    ...
    case 'number': {
      if (/count|size|total|num|length|estimate/.test(n)) return randInt(rand, 1, 10)
      if (/page|offset|index/.test(n)) return 0
 ...
 */
export function synthesizeFromOutputFields(
  outputFields: OutputField[] | undefined,
  seed: string,
): Record<string, unknown> {
  if (!outputFields || outputFields.length === 0) return {}
 ...
 */
export function createDefaultMock(outputFields: OutputField[] | undefined): MockActionHandler {
  return async (_args: Record<string, unknown>, ctx: MockActionContext) => {
